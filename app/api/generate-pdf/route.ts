...
export async function POST(request: NextRequest) {
  try {
    ...
    // Date
    addText(`Date: ${new Date().toLocaleDateString("en-IN")}`, 10)
    yPosition += 5

    // Section 1: Personal Details
    addText("1. PERSONAL DETAILS:", 11, "bold")
    addText(`   Name: ${fullName || "___________________________________________"}`, 10)
    addText(`   Email: ${email || "___________________________________________"}`, 10)
    addText(`   Phone: ${phone || "___________________________________________"}`, 10)

    yPosition += 3

    // Section 2: Company Details
    addText("2. COMPANY DETAILS:", 11, "bold")
    addText(`   Company Name: ${company || "___________________________________________"}`, 10)
    ...
    return NextResponse.json(
      { pdfUrl: pdfBase64, fileName: `BAA-Membership-Form-${fullName || "Application"}.pdf` },
      { status: 200 }
    )
  } catch (error) {
